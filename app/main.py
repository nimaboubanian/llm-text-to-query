...
from database_config import (
    DatabaseConfigManager, DatabaseType,
    discover_available_servers, get_server_databases, build_connection_url
)
...
def get_sql_from_llm(user_query, schema_str, db_type: DatabaseType = DatabaseType.POSTGRESQL):
...
def main():
    ...
    if all_databases:
        ...
        db_labels = [f"{name} ({all_databases[key].db_type.value})" for key, name in db_display_names]
    ...
    if servers:
        ...
        server_names = ["-- Select server --"] + [f"{s.name} ({s.host}:{s.port})" for s in servers]
        selected_server_name = st.sidebar.selectbox(
            "Database Server",
            options=server_names,
            key="quick_connect_server"
        )
        ...
        if selected_server_name != "-- Select server --":
            ...
            if databases:
                selected_database = st.sidebar.selectbox(
                    "Database/Schema",
                    options=databases,
                    key="quick_connect_database"
                )
                
                # Build connection URL
                connection_url = build_connection_url(selected_server, selected_database)
        ...
        st.sidebar.info("No database servers detected. Start containers or add manually.")
        ...
        with col1:
            if st.button("Test", key="test_conn"):
                if custom_db_url:
                    ...
                    success, error = validate_database_connection(custom_db_url, db_type_enum)
                    if success:
                        st.success("✅ OK")
                    else:
                        st.error(f"❌ {error}")
    ...
    if st.session_state.selected_db_key:
        current_db_config = db_config_manager.get_database(st.session_state.selected_db_key)
    ...
    st.info(f"🗄️ **Connected to:** {current_db_config.name} ({current_db_config.db_type.value})")
    ...
    if st.session_state.current_schema is None and not st.session_state.use_manual_schema:
        try:
    ...
    active_schema = (
        ...
        if st.session_state.use_manual_schema and st.session_state.manual_schema_override
    ...
    if query:
        ...
        with st.spinner("Generating SQL..."):
            generated_sql, error = get_sql_from_llm(query, active_schema, current_db_config.db_type)
